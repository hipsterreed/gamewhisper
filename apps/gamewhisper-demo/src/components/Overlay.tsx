...
export function Overlay({ isOpen, agentId, onClose }: OverlayProps) {
  ...
  // Auto-close 3s after session timeout
  useEffect(() => {
  ...
  return (
    <div className={`fixed top-6 left-1/2 -translate-x-1/2 w-[360px] z-50 ${isOpen ? 'overlay-enter' : 'overlay-exit'}`}>
      <div
        className="flex flex-col rounded-2xl border border-white/[0.08] select-none overflow-hidden"
        style={{
